...
<div class="container">
  ...
  <%      
			...
			String sql2="select czy_sid,SUM(czy_credit) 总学分 from Chenzy_gradeall where czy_sid="+(Integer)session.getAttribute("username")+"and czy_grade>60 group by czy_sid";
			Statement stmt=conn.createStatement();
			ResultSet rs1=stmt.executeQuery(sql2);
			if(rs1.next()){
			%>
	<div class="text-primary">
	你的已修学分是<%=rs1.getString("总学分") %>,以下是你的所有课程
	</div>
	<%} %>
	<%
			Statement sm=conn.createStatement();
			ResultSet rs2=sm.executeQuery("select count(*) from Chenzy_lessoncredit where czy_sid="+(Integer)session.getAttribute("username"));
			if(rs2.next()){
				recordCount=rs2.getInt(1);
			}
			pageCount=(recordCount%pageSize==0)?(recordCount/pageSize):(recordCount/pageSize+1);
			String sql="select top 10 czy_lname,czy_credit from Chenzy_lessoncredit where czy_sid="+(Integer)session.getAttribute("username")+"and czy_lname not in (select top "+(showPage-1)*pageSize+" czy_lname from Chenzy_lessoncredit)";
